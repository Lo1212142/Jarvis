...
class TestMemoryServiceWiring:
    ...
    def test_chat_uses_server_model_when_client_omits_model(self):
        engine = _make_engine(content="server-selected")
        app = create_app(engine, "server-default-model", config=_test_config())
        client = TestClient(app)

        resp = client.post(
            "/v1/chat/completions",
            json={"messages": [{"role": "user", "content": "Use the configured model"}]},
        )

        assert resp.status_code == 200
        assert engine.generate.call_args.kwargs["model"] == "server-default-model"
