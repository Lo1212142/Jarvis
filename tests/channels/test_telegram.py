...
class TestSend:
    ...
    def test_send_voice_respects_allowlist_and_sanitizes_filename(self):
        ch = TelegramChannel(bot_token="123:ABC", allowed_chat_ids="42")
        mock_response = MagicMock()
        mock_response.status_code = 200
        with patch("httpx.post", return_value=mock_response) as mock_post:
            assert ch.send_voice("42", b"audio", filename="../../voice.ogg", caption="Hi") is True
            assert mock_post.call_args.kwargs["data"]["chat_id"] == "42"
            assert mock_post.call_args.kwargs["files"]["voice"][0] == "voice.ogg"
        with patch("httpx.post") as mock_post:
            assert ch.send_voice("99", b"audio") is False
            mock_post.assert_not_called()

    def test_send_respects_allowlist(self):
        ch = TelegramChannel(bot_token="123:ABC", allowed_chat_ids="42")
        with patch("httpx.post") as mock_post:
            assert ch.send("99", "Hello!") is False
            mock_post.assert_not_called()
...
class TestVoiceDownload:
    def test_download_voice_is_bounded_and_does_not_store_to_disk(self):
        ch = TelegramChannel(bot_token="123:ABC", allowed_chat_ids="42")
        info = MagicMock(status_code=200)
        info.json.return_value = {"result": {"file_path": "voice/file.ogg"}}
        payload = MagicMock(status_code=200, content=b"audio-bytes")
        with patch("httpx.get", side_effect=[info, payload]) as get:
            assert ch.download_voice("file-id") == b"audio-bytes"
            assert get.call_count == 2
        traversal = MagicMock(status_code=200)
        traversal.json.return_value = {"result": {"file_path": "../escape.ogg"}}
        with patch("httpx.get", return_value=traversal) as get:
            assert ch.download_voice("file-id") is None
