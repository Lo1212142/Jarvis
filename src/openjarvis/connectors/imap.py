...
class IMAPConnector(GmailIMAPConnector):
    ...
    def sync(
        ...
        *,
        ...
        # Keep an explicit reference to the delegated generator.  Closing an
        # outer generator does not reliably close an iterator consumed by a
        # plain `for` loop, so close it in our own finally block to guarantee
        # IMAP logout on cancellation or partial iteration.
        delegated = super().sync(since=since, cursor=cursor)
        try:
            for doc in delegated:
