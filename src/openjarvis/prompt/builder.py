...
from openjarvis.prompt.jarvis_policy import get_jarvis_operating_policy
...
class SystemPromptBuilder:
    ...
    def _build_frozen_sections(self) -> list[PromptSection]:
        ...
        style = self._style_prompt_section()
        if style:
            sections.append(
                PromptSection(
                    name="interaction_style",
                    content=style,
                    source="system_prompt.persona_style",
                    cache_segment="frozen_prefix",
                )
            )
        if self._jarvis_mode or self._sp_config.jarvis_capability_guide_enabled:
            guide = get_jarvis_operating_policy()
            max_chars = max(1000, int(self._sp_config.jarvis_capability_guide_max_chars))
            sections.append(
                PromptSection(
                    name="jarvis_capability_guide",
                    content=self._truncate(guide, max_chars),
                    source="openjarvis.prompt.jarvis_policy",
                    cache_segment="frozen_prefix",
                )
            )
    ...
    def _style_prompt_section(self) -> str:
        """Build bounded humor and prediction rules from configuration."""
        cfg = self._sp_config
        if cfg.humor_enabled is None and cfg.predictive_suggestions is None:
            return ""
        lines = ["## Interaction Style"]
        if cfg.humor_enabled is True:
            style = cfg.humor_style or "dry"
            level = cfg.sarcasm_level or "light"
            lines.append(
                f"Use concise, context-aware {style} humor when appropriate; "
                f"sarcasm level is {level}."
            )
            lines.append(
                "Never let jokes obscure an answer, create ambiguity, or mock the user."
            )
        elif cfg.humor_enabled is False:
            lines.append("Do not use jokes or sarcasm unless the user explicitly asks for them.")
        if cfg.never_joke_about_sensitive_topics:
            lines.append(
                "Never joke about health crises, grief, disability, abuse, identity, "
                "financial distress, legal trouble, or other sensitive situations."
            )
        if cfg.predictive_suggestions is True:
            max_suggestions = max(0, int(cfg.max_predictive_suggestions))
            lines.append(
                f"Offer at most {max_suggestions} proactive suggestions when they are "
                "strongly supported by authorized context, and explain why each is relevant."
            )
            lines.append(
                "Predictions are suggestions only: ask before sending, deleting, buying, "
                "publishing, changing accounts, or performing other external side effects."
            )
        elif cfg.predictive_suggestions is False:
            lines.append("Do not proactively predict or suggest next actions unless asked.")
        return "\n".join(lines)
