...
class LoopGuard:
    ...
    def __init__(self, config: LoopGuardConfig, *, bus: Optional[EventBus] = None):
        ...
        # Emit one trigger event per active cycle; repeated blocked calls should
        # not flood the event bus or incident log.
        self._emitted_triggers: set[tuple[str, str]] = set()
    ...
    def _emit_triggered(self, reason_type: str, tool_name: str) -> None:
        """Publish one LOOP_GUARD_TRIGGERED event per active cycle."""
        trigger_key = (reason_type, tool_name)
        if trigger_key in self._emitted_triggers:
            return
        self._emitted_triggers.add(trigger_key)
        if self._bus:
            self._bus.publish(
