...
def _speech_provider_failure(operation: str) -> HTTPException:
    """Return a client-safe provider failure without exposing upstream details."""
    return HTTPException(
        status_code=502,
        detail=f"Speech {operation} is unavailable. Check the server speech provider configuration and server logs.",
    )


@speech_router.post("/synthesize")
async def synthesize_speech(payload: SpeechSynthesisRequest, request: Request):
    """Synthesize text with a registered TTS backend and return audio bytes."""
    text = payload.text.strip()
    if not text or len(text) > 20_000:
        raise HTTPException(status_code=422, detail="text must be 1-20000 characters")
    if not 0.25 <= payload.speed <= 4.0:
        raise HTTPException(status_code=422, detail="speed must be between 0.25 and 4.0")
    if payload.output_format not in {"mp3", "wav", "ogg", "opus"}:
        raise HTTPException(status_code=422, detail="unsupported output_format")

    import openjarvis.speech  # noqa: F401
    from openjarvis.core.registry import TTSRegistry

    aliases = {"openai": "openai_tts", "cartesia": "cartesia", "kokoro": "kokoro"}
    backend_key = aliases.get(payload.backend, payload.backend)
    if not TTSRegistry.contains(backend_key):
        raise HTTPException(status_code=501, detail=f"TTS backend '{payload.backend}' not available")
    try:
        ...
        raise _speech_provider_failure("synthesis") from exc
    media_types = {"mp3": "audio/mpeg", "wav": "audio/wav", "ogg": "audio/ogg", "opus": "audio/opus"}
    return Response(
        content=result.audio,
        media_type=media_types.get(result.format, "application/octet-stream"),
        headers={"X-Speech-Backend": backend_key, "X-Voice-Id": result.voice_id or payload.voice_id},
    )
...
async def transcribe_speech(request: Request):
    ...
    if len(audio_bytes) > 8 * 1024 * 1024:
        raise HTTPException(status_code=413, detail="Uploaded audio exceeds the 8MB limit")
    ...
    try:
        ...
        raise _speech_provider_failure("transcription") from exc
...
async def speech_health(request: Request):
    ...
    try:
        ...
        available = False
        reason = "Speech health check failed. Check server logs."

    if not available and reason is None:
        reason = "Speech backend is unavailable. Check server logs."
