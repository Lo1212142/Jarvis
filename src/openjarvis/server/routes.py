...
async def chat_completions(request_body: ChatCompletionRequest, request: Request):
    ...
    model = request_body.model or str(getattr(request.app.state, "model", "") or "")
    if not model:
        raise HTTPException(status_code=503, detail="No server chat model is configured")
    ...
    notice = _take_resource_notice(request.app.state, query_text_for_complexity)
    if notice:
        _prepend_response_notice(response, notice)
...
def _is_arabic_text(text: str) -> bool:
    return any(chr(0x0600) <= char <= chr(0x06ff) for char in text)


def _take_resource_notice(app_state: Any, user_text: str) -> str:
    monitor = getattr(app_state, "resource_monitor", None)
    if monitor is None:
        return ""
    try:
        alerts = monitor.alerts(10)
    except Exception:
        return ""
    if not alerts:
        return ""
    delivered_at = float(getattr(app_state, "resource_chat_alert_delivered_at", 0.0))
    fresh = [alert for alert in alerts if float(alert.get("timestamp", 0.0)) > delivered_at]
    if not fresh:
        return ""
    latest = fresh[-1]
    app_state.resource_chat_alert_delivered_at = float(latest.get("timestamp", 0.0))
    message = str(latest.get("message", "Resource usage is elevated."))
    if _is_arabic_text(user_text):
        return f"يا Boss، تنبيه موارد مبني على قياس حقيقي: {message}"
    return f"Boss, resource alert based on a real measurement: {message}"


def _prepend_response_notice(response: Any, notice: str) -> None:
