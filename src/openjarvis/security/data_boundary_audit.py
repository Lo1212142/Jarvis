...
# Includes explicit remote-client commands and bounded read-only discovery
# fetches. They are network egress, even when no cloud model is involved.
...
def _has_cloud_api_surface(config: Any | None, active_tools: set[str]) -> bool:
    ...
    if any(os.environ.get(name) for name in API_KEY_ENV_VARS):
        # A process may carry an ambient provider key (for example the
        # developer's shell) while the configured target is explicitly local.
        # Do not label that local target as cloud merely because an unused key
        # exists; active provider/tool signals are audited separately.
        if config is None:
            return True
        effective_engine = _first_nonempty(
            _get(config, "intelligence.preferred_engine", ""),
        ...
        default_model = _get(config, "intelligence.default_model", "")
        if not (default_model and _is_local_engine_value(effective_engine)):
            return True
