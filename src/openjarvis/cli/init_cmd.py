...
"""
    ...
    if not model:
        # Network-backed engines can still use a small, explicit fallback
        # model.  Keep the warning, but do not silently remove the install
        # path from `jarvis init` on low-memory systems.
        if selected_engine in ("ollama", "vllm", "sglang"):
            model = "qwen3.5:2b"
            console.print(
                "\n  [yellow]! Hardware estimate is below the local-model tier; "
                "offering the smallest configured model.[/yellow]"
            )
        else:
            console.print(
    ...
    if model:
        ...
        # Ollama and OpenAI-compatible servers can accept a model id without
        # local catalog metadata.  Only file-based engines require a catalog
        # spec before offering a download action.
        download_capable = selected_engine in ("ollama", "vllm", "sglang") or spec
        if not no_download and not from_bare_jarvis and download_capable:
